...
while True:
  ...
  if choice == "1":
    while True:
        ...
        item = input("Ievadiet ēdiena nosaukumu, ko pievienot: (press Enter to quit)").strip()
    ...
    if food:
        ...
        while True:
            ...
            if index.isdigit():  # Parbaudam lai butu ievaditi tikai cipari
                ...
                if index == -1:  # Ja lietotājs ievada 0, atceliet dzēšanu
                    ...
                    break
                elif 0 <= index < len(food):
                    removed_item = food.pop(index)
        ...
        # Saraksta garums
        print(f"Sarakstā ir {len(food)} vienumi")
    ...
    if len(food) > 0:
        print("Pašreizējie saraksta vienumi:")
        for i in range(len(food)):
            print(f"{i + 1}. {food[i]}")  # Izvedam numuru (i + 1 jo saraksts saksies ar 0) un edienu
    else:
